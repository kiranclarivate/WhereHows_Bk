...
public class FlowsDAO extends AbstractMySQLOpenSourceDAO {
  ...
  private final static String GET_FLOW_COUNT_BY_APP_ID_AND_PROJECT_NAME =
      "SELECT count(*) " + "FROM flow WHERE app_id = ? and flow_group = ?";

  private final static String GET_FLOW_COUNT_WITHOUT_PROJECT_BY_APP_ID =
      "SELECT count(*) " + "FROM flow WHERE app_id = ? and flow_group is null";
  ...
  private final static String GET_FLOW_TREE_APPLICATON_NODES =
      "SELECT DISTINCT ca.app_code " + "From flow f JOIN cfg_application ca ON f.app_id = ca.app_id ORDER by 1";

  private final static String GET_FLOW_TREE_PROJECT_NODES =
      "SELECT DISTINCT IFNULL(f.flow_group, 'ROOT') " + "FROM flow f JOIN cfg_application ca ON f.app_id = ca.app_id "
          + "WHERE (is_active is null or is_active = 'Y') and ca.app_code = ? ORDER BY 1";

  private final static String GET_FLOW_TREE_FLOW_NODES =
      "SELECT DISTINCT f.flow_id, f.flow_name FROM flow f " + "JOIN cfg_application ca ON f.app_id = ca.app_id "
          + "WHERE (f.is_active is null or f.is_active = 'Y') and ca.app_code = ? "
          + "and f.flow_group = ? ORDER BY f.flow_name";

  private final static String GET_FLOW_TREE_FLOW_NODES_WITHOUT_PROJECT =
      "SELECT DISTINCT " + "f.flow_id, f.flow_name FROM flow f " + "JOIN cfg_application ca ON f.app_id = ca.app_id "
          + "WHERE (f.is_active is null or f.is_active = 'Y') and ca.app_code = ? "
  ...
  public static Integer getApplicationIDByName(String applicationName) {
    ...
    try {
      applicationId =
          getJdbcTemplate().queryForObject(GET_APP_ID, new Object[]{applicationName.replace(".", " ")}, Integer.class);
    ...
    result = txTemplate.execute(new TransactionCallback<ObjectNode>() {
      public ObjectNode doInTransaction(TransactionStatus status) {
        ...
        for (Map row : rows) {
          ...
          flow.setAppCode((String) row.get("app_code"));
          if (StringUtils.isNotBlank(flow.getPath())) {
            int index = flow.getPath().indexOf(":");
            if (index != -1) {
              flow.setPath(flow.getPath().substring(0, index));
            }
          }
          Object created = row.get("created_time");
          if (created != null) {
            flow.setCreated(created.toString());
          }
          Object modified = row.get("modified_time");
          if (modified != null) {
            flow.setModified(row.get("modified_time").toString());
          }
      ...
      result = txTemplate.execute(new TransactionCallback<ObjectNode>() {
        public ObjectNode doInTransaction(TransactionStatus status) {
          ...
          if (StringUtils.isNotBlank(project) && (!project.equalsIgnoreCase("root"))) {
            rows = getJdbcTemplate().queryForList(GET_PAGED_FLOWS_BY_APP_ID_AND_PROJECT_NAME, applicationID, project,
                (page - 1) * size, size);
          } else {
            rows = getJdbcTemplate().queryForList(GET_PAGED_FLOWS_WITHOUT_PROJECT_BY_APP_ID, applicationID,
                (page - 1) * size, size);
          }
