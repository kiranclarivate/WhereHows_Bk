...
class OracleLoad:
  def __init__(self, args):
    ...
    self.input_table_file = os.path.join(temp_dir, args[Constant.GP_SCHEMA_OUTPUT_KEY])
    self.input_field_file = os.path.join(temp_dir, args[Constant.GP_FIELD_OUTPUT_KEY])
    self.input_sample_file = os.path.join(temp_dir, args[Constant.GP_SAMPLE_OUTPUT_KEY])
    self.collect_sample = False
    if Constant.GP_LOAD_SAMPLE in args:
      self.collect_sample = FileUtil.parse_bool(args[Constant.GP_LOAD_SAMPLE], False)
    self.logger.info("Load GreenPlum Metadata into {}, db_id {}, wh_exec_id {}"
  ...
  def load_tables(self):
    ...
    self.logger.info("finish loading greenplum table metadata from {}".format(self.input_table_file))
  ...
  def load_fields(self):
    ...
    self.logger.info("finish loading greenplum table fields from {}".format(self.input_field_file))
  ...
  def load_sample(self):
    ...
    self.logger.info("finish loading greenplum sample data from {}".format(self.input_sample_file))
  ...
  def run(self):
    try:
      ...
      self.logger.info("Load GreenPlum metadata [%s -> %s]" % (str(begin), str(end)))
    finally:
